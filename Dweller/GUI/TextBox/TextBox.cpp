...
namespace DUBGUI {
	...
	void TextBox::AppendStyles() {
		...
		for (unsigned int i = 0; i < LabelStrings.size(); i++) {
			...
			LabelStrings[i].setOutlineThickness(OutlineThickness);
		}
	...
	std::vector<std::wstring> TextBox::ParagraphBreak(std::wstring Str, unsigned int BlockSizeX) {
		...
		TextBufer.setOutlineThickness(OutlineThickness);
	...
	void TextBox::initialize(sf::RenderWindow* MainWindow, sf::Vector2u BlockSize) {
		...
		MouseProcessingObject.setSize(BlockSize);
	...
	TextBox::Status TextBox::update() {
		...
		TextBoxStatus = MouseProcessingObject.updateMouseButton(sf::Mouse::Left);
